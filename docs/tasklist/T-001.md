...
### Фаза 6: Telegram Bot
- [x] 6.1 Базовый бот с /start
- [x] 6.2 Команда /connect для привязки чата
...
**Критерии готовности:**
- [x] Бот запускается вместе с приложением (long polling)
- [x] /start отвечает приветствием с inline-кнопкой "Открыть приложение"
- [x] /start {inviteCode} — deep link для вступления
...
**Критерии готовности:**
- [x] /connect INVITE_CODE в групповом чате
- [x] Проверяет что отправитель — ADMIN команды
- [x] Сохраняет chat_id в таблицу teams
- [x] Отвечает "Чат привязан к команде X"
